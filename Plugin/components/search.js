async function handleSearchPage() {
  ...
  if (h1Element) {
    handleHeaderSearchPage(h1Element);
...
function expandButtonSearchPage() {
  ...
  if (!existExButton && areaEx) {
    ...
    console.error(
      "Não foi possível encontrar a área de inserção ou o botão já foi adicionado."
    );
  }
}
    ...
    toggleButton.addEventListener("click", () => {
      if (messageBox.style.display === "none") {
        ...
        chrome.runtime.sendMessage(
          { action: "fetchProductInfo", productId: productId },
          (response) => {
            if (response.error) {
              ...
              let priceText =
                document.querySelector(".sc-b0bec8a5-0.iSfaqD") ||
                document.querySelector(".sc-c3b80d8b-0.fKnFjg");
              ...
              const priceValue = parseFloat(
                priceText.replace("R$", "").trim().replace(",", ".")
              );
  ...
  for (let item of searchElements) {
    ...
    try {
      const response = await new Promise((resolve, reject) => {
        chrome.runtime.sendMessage(
          { action: "fetchCompetitorInfo", productId: productId },
          (response) => {
      ...
      const price = parseFloat(
        priceTag.replace(/R\$|\s/g, "").replace(",", ".")
      );
...
/**
 *
 * @param {HTMLHeadElement} h1Element
 */
async function handleHeaderSearchPage(h1Element) {
  const _h1Text = h1Element.textContent ?? "";
  const searchTerm = _h1Text.indexOf('"') ? _h1Text.split('"')[1] : _h1Text;
...
`;
  ...
  if (!has_content) {
    ...
    const totalProducts =
      document.querySelector(".sc-35785f40-0.kcCeq > span")?.textContent ?? "0";

    console.log(`${totalProducts} products found for term "${searchTerm}"`);
    ...
    for (const item of searchElements) {
      ...
      const productInfo = await apiCall("fetchProductInfo", productId).catch(
        (err) => err
      );
      if (productInfo instanceof Error) return console.error(productInfo);
      ...
      const monthS = !isNaN(parseFloat(productInfo.rbv))
        ? parseFloat(productInfo.rbv)
        : 0;
    ...
    contentDiv.innerHTML = `
                  ...
                  <div style="width:13.28%; height:100px; padding: 15px; margin-left:10px; border-radius: 10px;background-color: #FFF;border: 1px solid rgb(204, 204, 204);box-shadow: rgba(0, 0, 0, 0.1) 0px 2px 5px;color: rgb(51, 51, 51);align-self: center;display: block;text-align: center; overflow-wrap: break-word;"><i class="material-icons" style="font-size: 25px !important;!i;!;!;font-weight: 800 !important;!i;!;">attach_money</i><br><span style="font-size: 9px; color: gray;">Vendas hoje</span><br><br><span style="font-size: 10px;font-weight: 800;">${formatCurrency(
                    agg_todayS
                  )}</span></div>
                  <div style="width:13.28%; height:100px; padding: 15px; margin-left:10px; border-radius: 10px;background-color: #FFF;border: 1px solid rgb(204, 204, 204);box-shadow: rgba(0, 0, 0, 0.1) 0px 2px 5px;color: rgb(51, 51, 51);align-self: center;display: block;text-align: center; overflow-wrap: break-word;"><i class="material-icons" style="font-size: 25px !important;!i;!;!;font-weight: 800 !important;!i;!;">attach_money</i><br><span style="font-size: 9px; color: gray;">Vendas S-1</span><br><br><span style="font-size: 10px;font-weight: 800;">${formatCurrency(
                    agg_weekS
                  )}</span></div>
                  <div style="width:13.28%; height:100px; padding: 15px; margin-left:10px; border-radius: 10px;background-color: #FFF;border: 1px solid rgb(204, 204, 204);box-shadow: rgba(0, 0, 0, 0.1) 0px 2px 5px;color: rgb(51, 51, 51);align-self: center;display: block;text-align: center; overflow-wrap: break-word;"><i class="material-icons" style="font-size: 25px !important;!i;!;!;font-weight: 800 !important;!i;!;">attach_money</i><br><span style="font-size: 9px; color: gray;">Vendas mês</span><br><br><span style="font-size: 10px;font-weight: 800;">${formatCurrency(
                    agg_monthS
                  )}</span></div>
